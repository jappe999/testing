...
var TransformControlsGizmo = function () {
    ...
    var pickerTranslate = {
        X: [
            [
                new Mesh(new CylinderBufferGeometry(0.2, 0, 1, 4, 1, false), matInvisible),
                [0.6, 0, 0],
                [0, 0, -Math.PI / 2],
            ],
        ],
    ...
    var pickerScale = {
        X: [
            [
                new Mesh(new CylinderBufferGeometry(0.2, 0, 0.8, 4, 1, false), matInvisible),
                [0.5, 0, 0],
                [0, 0, -Math.PI / 2],
            ],
        ],
        ...
        for (var i = 0; i < handles.length; i++) {
            ...
            if (this.mode === "translate" || this.mode === "scale") {
                ...
                if (handle.name === "X" || handle.name === "XYZX") {
                    if (
                        Math.abs(alignVector.copy(unitX).applyQuaternion(quaternion).dot(this.eye)) > AXIS_HIDE_TRESHOLD
                    ) {
                ...
                if (handle.name === "Y" || handle.name === "XYZY") {
                    if (
                        Math.abs(alignVector.copy(unitY).applyQuaternion(quaternion).dot(this.eye)) > AXIS_HIDE_TRESHOLD
                    ) {
                        handle.scale.set(1e-10, 1e-10, 1e-10)
                        handle.visible = false
                    }
                ...
                if (handle.name === "Z" || handle.name === "XYZZ") {
                    if (
                        Math.abs(alignVector.copy(unitZ).applyQuaternion(quaternion).dot(this.eye)) > AXIS_HIDE_TRESHOLD
                    ) {
                        handle.scale.set(1e-10, 1e-10, 1e-10)
                        handle.visible = false
                    }
